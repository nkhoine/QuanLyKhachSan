...
namespace Nhom5_QuanLyKhachSan
{
    public static class Database
    {
        ...
        public static void SetCurrentUserInfo(string maNV, string hoTen)
        {
            try
            {
                using (var conn = GetConnection())
                {
                    ...
                    using (var cmd = new OracleCommand("BEGIN PKG_SECURITY.SET_USER_ID(:p_manv); END;", conn))
                    {
            ...
            {
                MessageBox.Show("Lỗi set context người dùng: " + ex.Message);
            }
        }
        ...
        public static string ConnectionString =
            ...
            "(CONNECT_DATA=(SERVICE_NAME=orcl)));User Id=QUANLYKS;Password=123456;";

        // NEW: Lưu mã nhân viên đang đăng nhập (phục vụ VPD)
        public static string CurrentMaNV { get; private set; }

        /// <summary>
        /// Gọi hàm này sau khi đăng nhập thành công để lưu mã nhân viên
        /// </summary>
        public static void SetCurrentMaNV(string maNV)
        {
            CurrentMaNV = maNV?.Trim();
        }

        /// <summary>
        /// Hàm nhận thông tin từ Form Login (nếu cho phép sửa host/port/sid/user/pass)
        /// </summary>
        ...
        /// <summary>
        /// Hàm nội bộ: áp dụng context VPD cho session hiện tại (nếu có mã NV)
        /// </summary>
        private static void ApplyContext(OracleConnection conn)
        {
            try
            {
                if (!string.IsNullOrEmpty(CurrentMaNV))
                {
                    using (OracleCommand ctxCmd = new OracleCommand(
                        "BEGIN PKG_SECURITY.SET_USER_ID(:P_MANV); END;",   // ⚠️ CHỈ GỌI PACKAGE, KHÔNG GỌI DBMS_SESSION
                        conn))
                    {
                        ctxCmd.BindByName = true;
                        ctxCmd.Parameters.Add("P_MANV", OracleDbType.Varchar2).Value = CurrentMaNV;
            ...
            {
                MessageBox.Show("Lỗi set context VPD: " + ex.Message);
            }
        }
